...
<p align="center">
  <a href="https://github.com/ZhaoHongYu1996/open-ai-canvas">GitHub</a> ·
...
当前版本以仓库根目录 [`VERSION`](VERSION) 为准。本仓库默认开发与部署分支是 `Dev`。
...
脚本会从 [ZhaoHongYu1996/open-ai-canvas](https://github.com/ZhaoHongYu1996/open-ai-canvas) 的 `Dev` 分支拉取源码。默认访问 `http://服务器IP:3000`。第一个注册账号会成为管理员；公开注册默认关闭。更新或排查：
...
```bash
cd /opt/open-ai-canvas
curl -fsSL https://raw.githubusercontent.com/ZhaoHongYu1996/open-ai-canvas/Dev/scripts/install-host-updater.sh | sudo bash
